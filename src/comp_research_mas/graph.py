...
def step3_research_adapter_node(state: WorkflowState) -> WorkflowState:
    ...
    save_raw_results(raw_results)
    return {**state, "raw_results": raw_results, "status": "researched_step3"}
...
def analyst_node(state: WorkflowState) -> WorkflowState:
    ...
    if not evidence:
        return {**state, "analysis_bundle": None, "writer_directives": ["AnalysisBundle empty: evidence fallback"], "status": "analysis_fallback"}
    ...
    return {**state, "analysis_bundle": bundle.to_dict(), "writer_directives": directives, "analysis_path": str(analysis_path), "status": "analyzed"}
...
def run_step3(week_id: str = "2026-26") -> WorkflowState:
    return build_step3_graph().invoke({"week_id": week_id, "iteration": 0, "error_log": []})
